#line 1 "c:\\Users\\guoob\\Desktop\\Hat\\hat\\test\\output\\sketch\\hatlib.cpp"
#line 1 "x:\\My Drive\\Apps\\Hat\\hat\\hatlib.cpp"
#include "hatlib.h"

#define NUM_PIXELS				512 //8 x 64 pixels
#define PIXEL_ROWS 				8
#define PIXEL_COLS 				64
#define PIXEL_DATA_PIN 			3
...
HatLib::HatLib(){
	...
	FastLED.addLeds<WS2812, PIXEL_DATA_PIN, PIXEL_COLOUR_ORDER>(pixelArray, NUM_PIXELS);
...
void HatLib::updatePixels(uint8_t newPixels[][3]){
...
void HatLib::displayPixels(PIXEL_SHIFT_DIRECTION shiftDirection){
	 ...
	 while(true){
		...
		//shift one over if the next position is invalid
		if(absoluteIndex + PIXEL_COLS >= NUM_PIXELS || absoluteIndex - PIXEL_COLS < 0){
			...
			absoluteIndex += PIXEL_COLS * jumpRight;
		}
	 }	
